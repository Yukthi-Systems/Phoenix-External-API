...
#[derive(Deserialize)]
pub struct MailBoxQuotaUpdateRequest {
    pub email: String,
    pub domain_name: String,
    pub new_quota_allocated: f64,
}
...
impl MailBoxQuotaUpdateRequest {
    pub fn validate(&self) -> Result<(), AppError> {
        if self.email.trim().is_empty() {
            return Err(AppError::BadRequest("Email cannot be empty".into()));
        }
        ...
        // Check if the new quota allocated is a positive number
        if self.new_quota_allocated < 0.1 {
            return Err(AppError::BadRequest("New quota allocated must be a positive number greater than 0.1".into()));
        }
